...
// فایل: js/main.js - این کد را به جای بلوک if (pageName === 'profile') فعلی قرار دهید
    ...
    if (pageName === 'profile') {
        ...
        // فراخوانی توابع مخصوص این صفحه
        lucide.createIcons();
        renderMyTasks(employee);
    }
    // --- بخش دایرکتوری (تیمی) ---
    ...
    portalContainer.innerHTML = `
        <div class="flex h-screen" style="background:#F5F6FA;">
            <aside class="w-72 employee-sidebar hidden sm:flex z-30">
                <div class="text-center"><img src="${employee.avatar}" alt="Avatar" class="profile-pic object-cover"><h2 class="employee-name">${employeeName}</h2><p class="employee-title">${employee.jobTitle || 'بدون عنوان شغلی'}</p></div><div class="my-6 border-t border-white/20"></div><nav id="employee-portal-nav" class="flex flex-col gap-2"><a href="#profile" class="nav-item active"><i data-lucide="layout-dashboard"></i><span>مسیر من</span></a><a href="#requests" class="nav-item"><i data-lucide="send"></i><span>کارهای من</span></a><a href="#directory" class="nav-item"><i data-lucide="users"></i><span>تیم‌ها</span></a><a href="#documents" class="nav-item"><i data-lucide="folder-kanban"></i><span>دانش‌نامه</span></a><a href="#inbox" class="nav-item"><i data-lucide="inbox"></i><span>پیام‌ها</span></a><a href="#moments" class="nav-item"><i data-lucide="sparkles"></i><span>لحظه‌های نیک‌اندیشی</span></a></nav><div class="mt-auto space-y-4"><button id="portal-logout-btn" class="w-full flex items-center justify-center gap-3 px-4 py-2 rounded-lg logout-btn"><i data-lucide="log-out"></i><span>خروج از حساب</span></button></div>
            </aside>
...
// فایل: js/main.js - event listener فرم را با این نسخه جایگزین کنید
...
        const selectedTeam = state.teams.find(t => t.firestoreId === selectedTeamId);
        const managedTeamId = document.getElementById('managed-team-select').value; // [!code ++]
...
        };
        ...
        const batch = writeBatch(db); // [!code ++]

        if (isEditing) {
            try {
                const docRef = doc(db, `artifacts/${appId}/public/data/employees`, emp.firestoreId);
                batch.update(docRef, employeeCoreData); // [!code focus]
                
                // [!code start]
                // بخش جدید: آپدیت کردن مدیر تیم
                if (managedTeamId) {
...
                    employeeData: { ...employeeCoreData, avatar: `...`, personalInfo: { email } },
                    teamId: selectedTeamId
                });
                
                // [!code start]
                // اگر کاربر همزمان به عنوان مدیر یک تیم هم انتخاب شده بود
