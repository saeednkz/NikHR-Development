...
function renderEmployeePortalPage(pageName, employee) {
    ...
    if (pageName === 'team-dashboard') {
        ...
        // کدی که شما فرستادید به طور کامل اینجا قرار می‌گیرد
        if (!isTeamManager(employee)) {
            contentContainer.innerHTML = `<div class="card p-6 text-center"><p>شما دسترسی به این صفحه را ندارید.</p></div>`;
            return;
        }
...
// ... داخل mainContent.addEventListener('click', ...)
        ...
        if (postBtn) {
            ...
            if (commentToggle) {
                ...
                const box = document.getElementById(`moment-comments-${id}`);
                if (box) box.classList.toggle('hidden');
                return;
            }
            // ارسال نظر
            const commentSend = e.target.closest('.moment-comment-send');
            if (commentSend) {
                (async () => {
                    try {
                        const id = commentSend.dataset.id;
                        const input = document.getElementById(`moment-comment-input-${id}`);
                        const text = (input?.value || '').trim();
                        if (!text) return;
                        const docRef = doc(db, `artifacts/${appId}/public/data/moments`, id);
                        const snap = await getDoc(docRef);
                        const data = snap.data() || {}; const comments = data.comments || [];
                        comments.push({ uid: employee.uid, name: employee.name, text, createdAt: new Date() });
                        await updateDoc(docRef, { comments, lastUpdatedAt: serverTimestamp() });
                        input.value = '';
                        showToast('نظر ثبت شد.');
                        window.renderMomentsList();
                    } catch (err) { showToast('خطا در ارسال نظر.', 'error'); }
                })();
                return;
            }
            // بازنشر
            const reshareBtn = e.target.closest('.moment-reshare-btn');
            if (reshareBtn) {
                (async () => {
                    try {
                        const id = reshareBtn.dataset.id;
                        const orig = (state.moments || []).find(m => m.firestoreId === id);
                        if (!orig) return;
                        await addDoc(collection(db, `artifacts/${appId}/public/data/moments`), {
                            ownerUid: employee.uid,
                            ownerName: employee.name,
                            text: orig.text ? `// بازنشر\n${orig.text}` : '',
                            imageUrl: orig.imageUrl || '',
                            reactions: [],
                            resharedFrom: { ownerName: orig.ownerName, createdAt: orig.createdAt },
                            createdAt: serverTimestamp()
                        });
                        showToast('بازنشر شد.');
                    } catch (err) { showToast('خطا در بازنشر.', 'error'); }
                })();
                return;
            }
...
function renderEmployeePortal() {
    ...
    ? `
        ...
        <a href="#tasks" class="nav-item"><i data-lucide="check-square"></i><span>وظایف من</span></a> 
      ` 
    : '';
    ...
    // Replace the simple header with the full detailed header
    portalContainer.querySelector('header').innerHTML = `<div class="w-full py-4 px-0 sm:px-6 lg:px-8 flex justify-between items-center" style="padding-top: env(safe-area-inset-top);"><div class="flex items-center gap-3"><button id="portal-menu-btn" class="inline-flex sm:hidden items-center justify-center p-2 rounded-md bg-white/20 hover:bg-white/30 text-white" title="منو"><i data-lucide="menu" class="w-5 h-5"></i></button><img src="logo.png" alt="Logo" class="w-8 h-8 rounded-md ring-2 ring-white/30"><div class="w-10 h-10 rounded-full overflow-hidden ring-2 ring-white/30"><img src="${employee.avatar}" alt="${employeeName}" class="w-full h-full object-cover"></div><div><div class="text-white/80 text-xs">خوش آمدید</div><h1 class="text-2xl font-bold text-white">${employeeName}</h1></div></div><div class="flex items-center gap-2"><button id="quick-new-request-btn" class="hidden sm:inline-flex items-center gap-2 text-xs font-semibold bg-white/15 hover:bg-white/20 text-white px-3 py-2 rounded-lg transition"><i data-lucide="plus" class="w-4 h-4"></i><span>ثبت درخواست</span></button><div class="relative sm:hidden"><button id="mobile-options-btn" class="p-2 rounded-full text-white hover:bg-white/20 transition-colors"><i data-lucide="more-vertical" class="w-5 h-5"></i></button><div id="mobile-options-dropdown" class="hidden absolute left-0 mt-2 w-48 bg-white rounded-lg shadow-xl z-30 text-slate-700"><a href="#" id="mobile-new-request" class="flex items-center gap-3 px-4 py-2 border-t hover:bg-slate-100 text-sm"><i data-lucide="plus" class="w-4 h-4"></i><span>ثبت درخواست</span></a></div></div><div id="portal-notification-bell-wrapper" class="relative"><button id="portal-notification-bell-btn" class="relative cursor-pointer p-2 rounded-full hover:bg-white/10"><i data-lucide="bell" class="text-white"></i><span id="portal-notification-count" class="hidden absolute -top-1 -right-1 text-white text-xs w-5 h-5 flex items-center justify-center rounded-full border-2 border-white" style="background:#FF2E63"></span></button></div></div></div>`;
...
/**
 * Finds the correct approver's UID for a given employee.
 * If the employee is a manager, it finds their manager from the leadership team.
 * If the employee is not a manager, it finds their direct manager from their primary team.
...
async function showRequestDetailsModal(requestId, employee) {
    ...
    if (request.requestType === 'تایید مهارت') {
        const targetEmployee = state.employees.find(e => e.firestoreId === request.context?.employeeId);
        modalContent.innerHTML = `
            <div class="space-y-4">
                <div class="p-4 border rounded-lg bg-slate-50 text-sm">
        ...
        document.getElementById('reject-skill-btn')?.addEventListener('click', async (e) => {
             e.currentTarget.disabled = true;
             await updateDoc(doc(db, `artifacts/${appId}/public/data/requests`, requestId), { status: 'رد شده' });
             showToast('درخواست رد شد.');
             closeModal(mainModal, mainModalContainer);
        });

    } else {
        // Original logic for all other requests (unchanged)
        const threadHtml = (request.thread || []).map(item => {
            const sender = state.users.find(u => u.firestoreId === item.senderUid)?.name || 'کاربر';
            const dateTxt = item.createdAt?.toDate ? toPersianDate(item.createdAt) : '';
            return `<div class="p-3 mt-2 text-sm border rounded-lg bg-slate-50"><p class="whitespace-pre-wrap">${item.content}</p><div class="text-slate-400 text-xs text-left mt-2">${sender} - ${dateTxt}</div></div>`;
        ...
        modalContent.innerHTML = `
            <div class="space-y-4">
                <div class="p-4 border rounded-lg bg-slate-50 text-sm">
                    <div class="flex justify-between items-center"><p><strong>موضوع:</strong> ${request.details}</p><span class="px-2 py-1 text-xs font-medium rounded-full bg-slate-100">${request.status}</span></div>
                </div>
                <div><strong class="text-slate-600">تاریخچه مکالمات:</strong><div class="mt-2 max-h-60 overflow-y-auto pr-2">${threadHtml}</div></div>
                <div class="pt-4 border-t">
...
const showEvaluationForm = (employee, cycle, evaluation) => {
    ...
    if (evaluation && evaluation.selfAssessment) {
        ...
        selfAssessmentHtml = `
            <div class="p-4 bg-slate-100 rounded-lg border text-sm text-slate-700 space-y-3">
                ...
                <div class="border-t pt-3">
                    <p class="font-bold mb-1">زمینه‌های قابل بهبود از دید کارمند:</p>
                    <p class="whitespace-pre-wrap">${selfData.areasForImprovement || '-'}</p>
                </div>
            </div>`;
    }
    ...
    document.getElementById('manager-evaluation-form')?.addEventListener('submit', async (e) => {
        ...
        const competencyScores = {};
        document.querySelectorAll('.competency-score').forEach(input => { competencyScores[input.dataset.name] = parseInt(input.value); });
        const avgCompetencyScore = Object.values(competencyScores).length > 0 ? Object.values(competencyScores).reduce((a, b) => a + b, 0) / Object.values(competencyScores).length : 0;

        const okrScores = {};
        document.querySelectorAll('.okr-score').forEach(input => { okrScores[input.dataset.title] = parseInt(input.value); });
        const avgOkrScore = Object.values(okrScores).length > 0 ? Object.values(okrScores).reduce((a, b) => a + b, 0) / Object.values(okrScores).length : 0;
    ...
    document.getElementById('skill-form').addEventListener('submit', async (e) => {
        ...
        try {
            ...
            setTimeout(() => {
                ...
                else viewEmployeeProfile(employee.firestoreId);
            }, 300);
...
// ▲▲▲ END: [REFACTOR - Phase 5 BUGFIX] Replace the entire function ▲▲▲
// ▲▲▲ END: [NEW FUNCTION - Phase 5] ▲▲▲
// ▼▼▼ START: [FINAL VERSION] Replace the entire showEmployeeForm function with this code ▼▼▼
const showEmployeeForm = (employeeId = null) => {
    ...
    modalContent.innerHTML = `
        <form id="employee-form" class="space-y-5" data-old-level="${emp.level || ''}" data-old-primary-team-id="${emp.primaryTeamId || ''}" data-employee-internal-id="${emp.id || ''}">
    ...
    document.getElementById('employee-form').addEventListener('submit', async (e) => {
        ...
        const employeeInternalId = form.dataset.employeeInternalId;

        const name = document.getElementById('name').value;
        const newEmployeeId = document.getElementById('id').value;
        const email = document.getElementById('employee-email').value;
        const newLevel = document.getElementById('level').value;
        const newPrimaryTeamId = document.getElementById('primary-team-select').value;
        const newTeamMemberships = Array.from(document.querySelectorAll('.team-membership-checkbox:checked')).map(cb => cb.value);

        const newPrimaryTeam = state.teams.find(t => t.firestoreId === newPrimaryTeamId);
        ...
        const employeeCoreData = {
            ...
            level: newLevel,
            primaryTeamId: newPrimaryTeamId,
            department: newPrimaryTeam ? newPrimaryTeam.name : '', // Legacy field for compatibility
        ...
        if (isEditing) {
            try {
                ...
                // ... (Career path logging logic can be added back here if needed) ...

                batch.update(employeeRef, employeeCoreData);

                // --- Handle Team Membership and Leadership Changes ---
                const isNowManager = newLevel.startsWith('MAN') || newLevel.startsWith('L');
                const wasManager = oldLevel.startsWith('MAN') || oldLevel.startsWith('L');

                // Case 1: Primary Team Changed
                if (newPrimaryTeamId !== oldPrimaryTeamId) {
                    // Remove from old primary team (either as manager or member)
                    if (oldPrimaryTeamId) {
                        ...
                        if (wasManager) {
                            ...
                            batch.update(oldTeamRef, { memberIds: arrayRemove(employeeInternalId) });
                        }
                    }
                    // Add to new primary team (either as manager or member)
                    if (newPrimaryTeamId) {
                        const newTeamRef = doc(db, `artifacts/${appId}/public/data/teams`, newPrimaryTeamId);
                        if (isNowManager) {
                            batch.set(newTeamRef, { leadership: { manager: newEmployeeId } }, { merge: true });
                        } else {
                            batch.update(newTeamRef, { memberIds: arrayUnion(newEmployeeId) });
                        }
                    }
                } else if (newPrimaryTeamId && isNowManager !== wasManager) {
                    // Case 2: Primary team is same, but role changed (e.g. promoted to manager)
                    const teamRef = doc(db, `artifacts/${appId}/public/data/teams`, newPrimaryTeamId);
                    if (isNowManager) { // Promoted to manager
                        batch.update(teamRef, { memberIds: arrayRemove(employeeInternalId) });
                        batch.set(teamRef, { leadership: { manager: newEmployeeId } }, { merge: true });
                    } else { // Demoted from manager
                        batch.update(teamRef, { 'leadership.manager': null });
                        batch.update(teamRef, { memberIds: arrayUnion(newEmployeeId) });
                    }
                }
                
                // Case 3: Handle secondary memberships
                const oldMemberships = new Set(emp.teamMemberships || []);
                const newMemberships = new Set(newTeamMemberships);
                
                // Remove from teams they are no longer a member of
                for (const oldTeamId of oldMemberships) {
                    if (!newMemberships.has(oldTeamId)) {
                        const teamRef = doc(db, `artifacts/${appId}/public/data/teams`, oldTeamId);
                        batch.update(teamRef, { memberIds: arrayRemove(employeeInternalId) });
                    }
                }
                // Add to new teams
                for (const newTeamId of newMemberships) {
                    if (!oldMemberships.has(newTeamId)) {
            ...
            try {
                ...
                await createNewEmployee({
                    ...
                    // The new cloud function will handle team assignments based on the data
                });
...
function showManageCategoryDocsModal(categoryKey) {
    ...
    const rows = docs.map(d => `
        <tr class="border-b">
            ...
            <td class="p-2 text-xs text-slate-500">${(d.tags||[]).join('، ')}</td>
        ...
        <div class="space-y-4">
            <div class="bg-slate-50 rounded-lg p-3">
                <form id="cat-doc-upload-form" class="grid grid-cols-1 md:grid-cols-2 gap-3 items-end">
                    ...
                    <div class="md:col-span-2">
                        <label class="block text-xs mb-1">برچسب‌ها (با ویرگول جدا کنید)</label>
                        <input id="cat-doc-tags" class="w-full p-2 border rounded-md" placeholder="مثال: آیین‌نامه، مرخصی">
                    </div>
                    <div class="md:col-span-2">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead class="bg-slate-100 text-slate-600">
                        <tr><th class="p-2 text-right">عنوان</th><th class="p-2 text-right">فایل</th><th class="p-2 text-right">برچسب‌ها</th><th class="p-2 text-right">گیرندگان</th><th class="p-2 text-right"></th></tr>
                    </thead>
    ...
    document.getElementById('cat-doc-upload-form').addEventListener('submit', async (e) => {
        ...
        const tags = (document.getElementById('cat-doc-tags').value || '').split(',').map(t=>t.trim()).filter(Boolean);
        ...
        try {
            ...
            await addDoc(collection(db, `artifacts/${appId}/public/data/companyDocuments`), {
                title, categoryKey: categoryKey, fileUrl: url, uploadedAt: serverTimestamp(), targets, tags
            });
...
function showViewCategoryDocsModal(categoryKey) {
    ...
    const itemsHtml = (docsList) => docsList.map(d => `
        <div class="flex items-center justify-between p-3 border-b last:border-b-0">
            <div>
                ...
                ${(d.tags||[]).length ? `<div class=\"mt-1 flex flex-wrap gap-1\">${(d.tags||[]).map(t=>`<span class=\"text-[10px] px-2 py-0.5 rounded-full bg-slate-100 text-slate-700\">${t}</span>`).join('')}</div>` : ''}
            </div>
            <div class="flex items-center gap-2">
    ...
    modalContent.innerHTML = `
        ...
        <div class="bg-white rounded-2xl border mb-3 p-3 flex flex-wrap items-center gap-2">
            <input id="docs-view-search" class="p-2 border rounded-lg text-xs" placeholder="جستجو در عنوان"/>
            <select id="docs-view-sort" class="p-2 border rounded-lg text-xs bg-white">
            ...
            <input id="docs-view-tags" class="p-2 border rounded-lg text-xs" placeholder="فیلتر برچسب‌ها (ویرگول‌دار)">
            <div class="text-[11px] text-slate-500 ml-auto">نتایج: <span id="docs-view-count">0</span></div>
        </div>
        <div id="docs-view-list" class="bg-white rounded-2xl border">
        </div>`;
    openModal(mainModal, mainModalContainer);
    const listEl = document.getElementById('docs-view-list');
    const countEl = document.getElementById('docs-view-count');
    const searchEl = document.getElementById('docs-view-search');
    const sortEl = document.getElementById('docs-view-sort');
    const tagsEl = document.getElementById('docs-view-tags');
    function applyDocFilters() {
        const q = (searchEl.value || '').trim();
        const sort = sortEl.value;
        const tags = (tagsEl.value || '').split(',').map(t=>t.trim()).filter(Boolean);
        let list = visibleDocs.filter(d => q ? (d.title||'').includes(q) : true);
        if (tags.length) list = list.filter(d => (d.tags||[]).some(t => tags.includes(t)));
        list = list.slice().sort((a,b)=>{
            const ad = new Date(a.uploadedAt?.toDate?.() || a.createdAt || 0);
            const bd = new Date(b.uploadedAt?.toDate?.() || b.createdAt || 0);
        ...
        listEl.innerHTML = itemsHtml(list) || '<div class="p-6 text-center text-slate-400 text-sm">موردی برای نمایش نیست.</div>';
    }
    searchEl.addEventListener('input', () => { clearTimeout(window._docDeb); window._docDeb = setTimeout(applyDocFilters, 250); });
    sortEl.addEventListener('change', applyDocFilters);
    tagsEl.addEventListener('input', () => { clearTimeout(window._docDeb2); window._docDeb2 = setTimeout(applyDocFilters, 300); });
    applyDocFilters();
}
